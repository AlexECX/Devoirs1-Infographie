// Transcrypt'ed from Python, 2018-09-10 17:42:16
...
export var divide_square = function (sq, count) {
	if (count === 0) {
		...
		points.push (...list ([sq [0], sq [1], sq [2]]));
		var vertices = points;
		var program = select_shaders (gl, 'vertex-shader', 'fragment-shader2');
		render (gl, program, gl.TRIANGLES, vertices);
		points.push (...list ([sq [1], sq [2], sq [3]]));
		var vertices = points;
		var program = select_shaders (gl, 'vertex-shader', 'fragment-shader2');
		render (gl, program, gl.TRIANGLES, vertices);
		points.push (...list ([sq [2], sq [3], sq [0]]));
		var vertices = points;
		var program = select_shaders (gl, 'vertex-shader', 'fragment-shader2');
		render (gl, program, gl.TRIANGLES, vertices);
		points.push (...list ([sq [3], sq [0], sq [1]]));
		var vertices = points;
		var program = select_shaders (gl, 'vertex-shader', 'fragment-shader2');
		render (gl, program, gl.TRIANGLES, vertices);
	}
		...
		divide_square (tuple ([ab, ba, ac, bd]), count);
		divide_square (tuple ([sq [1], ba, bc, bd]), count);
		divide_square (tuple ([bc, cb, bd, ca]), count);
		divide_square (tuple ([sq [2], cd, cb, ca]), count);
		divide_square (tuple ([cd, dc, ca, db]), count);
		divide_square (tuple ([sq [3], dc, da, db]), count);
		divide_square (tuple ([da, ad, db, ac]), count);
	}
};
